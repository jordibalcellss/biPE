...
$db = new DB();
$stmt = $db->prepare("SELECT COUNT(*) FROM time_log WHERE user_id=? AND saved=1");
...
if (TIMESHEET_RESULTS_PAGE < $num_records) {
  if ($page * TIMESHEET_RESULTS_PAGE < $num_records) {
    ...
    $switcher = "        <a href=\"?module=timesheet&page=$target_page_num\">".previous_page."</a>\n";
  }
  elseif ($page * TIMESHEET_RESULTS_PAGE > $num_records) {
    $target_page_num = $page - 1;
    $switcher = "        <a href=\"?module=timesheet&page=$target_page_num\">".next_page."</a>\n";;
  }
  else {
    $switcher = '';
  }
...
$stmt = $db->prepare("SELECT DATE_FORMAT(day,'%d-%m-%Y') AS day,
                      (CASE
                        ...
                        CASE
                          ...
                          ELSE CONCAT('<span class=\"code-p\">', tasks.code, \"</span> \", tasks.name)
                        END
...
if (count($records) > 0) {
  echo '      <h2>'.my_timesheet."</h2>\n";
  echo '      <div id="sec-menu"><a href="export.php">'.export_timesheet_csv."</a></div>\n";
  ...
  $stmt = $db->prepare("SELECT YEAR(day) AS year,
                        ...
                        (SELECT DISTINCT(YEAR(day)) FROM time_log WHERE user_id=? AND task_id=2 OR task_id=3 OR task_id=4 AND saved=1) 
                        AND user_id=? AND task_id=2 OR task_id=3 OR task_id=4 AND saved=1 GROUP BY YEAR(day), task_id");
  $stmt->execute(array($_SESSION['id'], $_SESSION['id']));
  $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
  
  //prepare table
  echo "      <div class=\"two-thirds alpha\">\n";
  echo "        <table>\n";
  echo "          <tr>\n";
  echo "            <th>".day."</th>\n";
  echo "            <th>".duration."</th>\n";
  ...
  foreach ($records as $record) {
    if ($record['duration'] == 0) {
      ...
      $duration = floor($record['duration']).' '.hours.decimalPartToFrac($record['duration']);
    }
    if (getLastRecord($_SESSION['id']) == new DateTime($record['day'], new DateTimeZone(TIMEZONE))) {
      $remove = '<a href="?module=timesheet&action=remove&id='.$record['record_id'].'">'.remove.'</a>';
    }
    else {
      $remove = '';
    }
  ...
  for ($i = 0; $i < count($rows); $i++) {
    if ($rows[$i]['year'] != $year_prev) {
      $to_close = !$to_close;
      echo "      <div class=\"one-third box-year\">\n";
      echo "        <h1>".$rows[$i]['year']."</h1>\n";
      echo "        <ul>\n";
      if (LOCALE == 'ca') {
        echo "          <li>".$rows[$i]['task'].": ".str_replace('.', ',', rtrim($rows[$i]['amount'], '0.'))." ".days."</li>\n";
      }   
      else {
        echo "          <li>".$rows[$i]['task'].": ".rtrim($rows[$i]['amount'], '0.')." ".days."</li>\n";
      }
    }
    else {
      if (LOCALE == 'ca') {
