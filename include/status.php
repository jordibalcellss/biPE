...
$stmt = $db->prepare("SELECT SUM(amount)
  FROM quotations WHERE task_id=? AND nature='i'");
$stmt->execute(array($_GET['id']));
$estimated_income = $stmt->fetchColumn();

$stmt = $db->prepare("SELECT SUM(amount)
  FROM quotations WHERE task_id=? AND nature='e'");
$stmt->execute(array($_GET['id']));
...
if ($rate) {
  echo '      <div class="one-quarter box-number alpha">'."\n";
  echo '        <h1>'.rate.' '.rate_math."</h1>\n";
  echo '        <span>'.formatNumberP($rate, false, true)."</span>\n";
  echo '      </div>'."\n";

  $stmt = $db->prepare('SELECT SUM(duration) AS spent
    FROM time_log WHERE task_id=? AND saved');
  $stmt->execute(array($_GET['id']));
  $spent = $stmt->fetchColumn();

  echo '      <div class="one-quarter box-number">'."\n";
  ...
  echo '        <span>'.number_format(round($remaining), 0, '', '.').
    "</span>\n";
  echo '      </div>'."\n";
...
$stmt = $db->prepare("SELECT SUM(amount)
  FROM invoices WHERE task_id=? AND nature='i'");
$stmt->execute(array($_GET['id']));
...
$stmt = $db->prepare("SELECT SUM(amount)
  FROM invoices WHERE task_id=? AND nature='e'");
$stmt->execute(array($_GET['id']));
...
$stmt = $db->prepare("SELECT SUM(amount)
  FROM invoices WHERE task_id=? AND nature='i' AND settled");
$stmt->execute(array($_GET['id']));
...
$stmt = $db->prepare("SELECT SUM(amount)
  FROM invoices WHERE task_id=? AND nature='e' AND settled");
$stmt->execute(array($_GET['id']));
